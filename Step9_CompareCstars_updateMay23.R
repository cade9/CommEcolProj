...
#should calculate and loop through number of samples
# out  directory
dirOut <- "E:\\cade\\CommEcolProj\\Output\\CompareAVIRIS"
cm.all3 <- NULL
nCount <- NULL 

# shrutilist 1 should be 4537 
...
for(x in 1:100){
  for(i in 1:length(shrutiCropList)){
    ...
    # create layer frequency
    layer.freq <- freq(shrutiClassCrop, useNA="no")
    n1 <- ceiling(min(layer.freq[,"count"]) * 0.002)
    n2 <- data.frame(iteration = iteration,
                        raster = names(shrutiClass),
                        numberSamples = n1)
    nCount <- rbind(n2,nCount)
    rSample <- sampleStratified(shrutiClassCrop, size = n1 ,xy = TRUE, sp=TRUE, na.rm = TRUE) 
    
    
    
    
    for(j in 1:length(rasterList)){
      multRast <- raster(rasterList[j]) #j
      NAvalue(multRast) <- 0
      ...
      cm <-capture.output(print(mod_RF_conMatrix), 
                          file = paste0(dirOut,"\\",rasterName,"_",subArea,"_CMoutputMay27_it_hyper",x,".txt"), append = TRUE) 
      ...
      cm.all= data.frame(rasterName, overall.accuracy, prod.EMR, user.EMR, prod.FLT, user.FLT, prod.SAV, user.SAV, prod.water, user.water, row.names= NULL)
